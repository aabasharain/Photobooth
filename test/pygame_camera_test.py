...
def take_pictures(surface, num_pics=3, total_countdown_seconds=3.0):
    ...
    for i in range(num_pics):
        ...
        while count > 0:
            ...
            circle_rect = pg.draw.circle(
                alpha_surface,
                pg.Color(255, 255, 255, 175),
                center_screen,
                int(size[0] / 16),
            )
...
def create_final_image(surface, imgs, print_dimensions=(2, 6), dpi=300):
    ...
    """
    img_positions = [(0, 0), (0, 455), (0, 455 * 2)]
    ...
    surface.fill(white)
    image_surface_scaled = pg.transform.scale(
        image_surface,
        (
            int(image_surface.get_rect().width * image_scale),
